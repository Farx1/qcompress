#!/usr/bin/env python3
"""
Script de benchmark minimal pour évaluer l'impact de la compression TT.
"""
...
def parse_args():
    parser = argparse.ArgumentParser(description="Benchmark minimal de compression TT")
    ...
    parser.add_argument('--output', type=str, default='benchmark_minimal_results.json', help='Output file')
...
def setup_device(device_arg):
    ...
    if device == 'mps':
        device = 'cpu'
        print("Forcing CPU to avoid MPS issues")
...
def measure_inference_speed(model, dataloader, device, num_runs=3):
    ...
    # Warmup
    for batch in list(dataloader)[:1]:
        batch = {k: v.to(device) for k, v in batch.items()}
        with torch.no_grad():
            _ = model(**batch)
...
def create_compressed_model(model):
    """Create a compressed version of the model by replacing lm_head with TT layer"""
    print("Creating compressed model...")
    
    # Create a copy of the model
    compressed_model = AutoModelForCausalLM.from_pretrained('gpt2')
    
    # Replace lm_head with TT version
    original_lm_head = compressed_model.lm_head
    in_features = original_lm_head.in_features  # 768
    out_features = original_lm_head.out_features  # 50257
    
    # Simple factorization - both must have same length
    in_modes = [16, 16, 3]  # 768 = 16*16*3
    out_modes = [17, 17, 17]  # 4913 ≈ 50257/10
    ranks = [1, 16, 16, 1]
    ...
    if original_lm_head.bias is not None and tt_layer.bias is not None:
        with torch.no_grad():
            tt_layer.bias.copy_(original_lm_head.bias)
    
    compressed_model.lm_head = tt_layer
    print(f"Replaced lm_head with TT layer")
